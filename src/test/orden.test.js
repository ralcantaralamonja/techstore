...
describe('Orden Controller', () => {
    ...
    it('should get an order by ID', async () => {
        ...
        // Mock the database response
        jest.spyOn(ordenMethods, 'getOrden').mockImplementation((req, res) => res.json(mockOrder[0]));
    ...
    it('should handle missing fields when updating an order', async () => {
        ...
        expect(response.status).toBe(400);
        expect(response.body).toHaveProperty('message', 'Bad Request. Please provide estado.');
    });
    ...
    it('should handle missing fields when deleting an order', async () => {
        ...
        expect(response.status).toBe(400);
        expect(response.body).toHaveProperty('message', 'Bad Request. ID is required.');
    });
});
